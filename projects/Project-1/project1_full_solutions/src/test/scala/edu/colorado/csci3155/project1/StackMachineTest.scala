...
class StackMachineTest extends FunSuite {
    ...
    test("stack machine test 6") {
        val lst1 = List(PushNumI(5.0), PushNumI(2.0), DivI)
        val (res, _) = StackMachineEmulator.emulateStackMachine(lst1)
        assert(res.getDoubleValue == 2.5)
    }

    test("personal"){
        val expr = IfThenElse(BoolConst(false), Const(1), Const(2))
        val compiled = compileToStackMachineCode(expr)
        val result = StackMachineEmulator.emulateStackMachine(compiled)
    }
