class MySuite extends munit.FunSuite {
  ...
  test("Problem 1 - Test 4") {
    val x0 = Alex(Andrew)
    val x1 = Emanuel(x0, Simon, Yohanes)
    val x2 = Gunn(x1, Harley)
    val x3 = Andrew
    for (x <- List(x0, x1, x2, x3)) {
        assert(x.isInstanceOf[Spencer])
        assert(!x.isInstanceOf[Abhishek])
        assert(!x.isInstanceOf[Varun])
        assert(!x.isInstanceOf[Ali])
  ...
  test("Problem 2A") {
    ...
    assertEquals({ t3 * 2 }, NodeVisitor(NodeVisitor(t0, 10, t0), 20, NodeVisitor(t0, 30, t0)))
  ...
  test("Problem 2B") {
    ...
    assertEquals({ t3 * 2 }, NodePattern(NodePattern(t0, 10, t0), 20, NodePattern(t0, 30, t0)))
    assertEquals({ t3.prod }, 750)
  ...
  test("Problem 2C") {
    ...
    assertEquals(t2Alt, NodeVisitor(EmptyTreeVisitor, 25, EmptyTreeVisitor))
  }
