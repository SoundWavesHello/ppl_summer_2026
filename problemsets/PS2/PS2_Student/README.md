...
4. **Very Important:** Your code will be implemented in **src/main/scala/Main.scala**. Tests are written for you in **src/test/scala/MySuite.scala**. These tests passing, along with adherence to the restrictions outlined for each problem, will determine your grade for the problem set.  We make most tests available to you, but we do use a few hidden test cases of our own as well.  We would encourage you to examine the differences between the test cases and the specifications provided for you if you are interested in figuring out what those hidden test cases are . . . 
