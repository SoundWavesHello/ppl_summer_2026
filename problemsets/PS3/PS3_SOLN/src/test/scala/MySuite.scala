class MySuite extends munit.FunSuite {
  ...
  test("insertTest4") {
    val ln = List(2270, 1300, 2270, 3155, 2275, 3155, 3155)
    val t0 = EmptyTree
    val t1 = TreeNode(t0, 2270, 1, t0)
    val t2 = TreeNode(TreeNode(t0, 1300, 1, t0), 2270, 1, t0)
    val t3 = TreeNode(TreeNode(t0, 1300, 1, t0), 2270, 2, t0)
    val t4 = TreeNode(TreeNode(t0, 1300, 1, t0), 2270, 2, TreeNode(t0, 3155, 1, t0))
    val t5 = TreeNode(TreeNode(t0, 1300, 1, t0), 2270, 2, TreeNode(TreeNode(t0, 2275, 1, t0), 3155, 1, t0))
    val t6 = TreeNode(TreeNode(t0, 1300, 1, t0), 2270, 2, TreeNode(TreeNode(t0, 2275, 1, t0), 3155, 2, t0))
    val t7 = TreeNode(TreeNode(t0, 1300, 1, t0), 2270, 2, TreeNode(TreeNode(t0, 2275, 1, t0), 3155, 3, t0))
    val lt = List(t0, t1, t2, t3, t4, t5, t6, t7)

    for ((n, (t, tp)) <- ln zip (lt zip lt.tail)) {
        assertEquals(t insert n, tp)
    }
  ...
  test("deleteMinTest3") {
    val t0 = EmptyTree
    val t1 = TreeNode(t0, 2270, 2, t0)
    assertEquals(t1.deleteMin(), TreeNode(t0, 2270, 1, t0))

    val t2 = TreeNode(t0, 3155, 15, t0)
    assertEquals(t2.deleteMin(), TreeNode(t0, 3155, 14, t0))
  }
  ...
  test("isValidSearchTreeTest1") {

    //base cases
    val t0 = EmptyTree
    assertEquals(t0.isValidSearchTree(), true)

    for (i <- -2 to 0) {
        val t1 = TreeNode(t0, 2270, i, t0)
        assertEquals(t1.isValidSearchTree(), false)
    }

    val i = 1
    val t2 = TreeNode(t0, 2270, i, t0)
    assertEquals(t2.isValidSearchTree(), true)
  }
  ...
  test("p2GrammarTest3") {
    val m0 = Plus(NumValue(Double.MinValue), NumValue(2.0))
    val m1 = Div(m0, NumValue(0.0))
    val m2 = Times(m1, NumValue(Double.MaxValue))
    val m3 = Minus(m2, NumValue(Double.NaN))
    val m4 = TryCatch(m3, NumValue(42))
    for (m <- m0 +: m1 +: m2 +: m3 +: m4 +: List()) {
        assert(!m.isInstanceOf[MathsError])
        assert(!m.isInstanceOf[Value])
        assert(m.isInstanceOf[Maths])
    }
  }
