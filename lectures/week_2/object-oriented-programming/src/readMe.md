The gameplan for the week:
- A brief crash course in OOP (today)
- An introduction to grammars
- Visitor pattern vs matching pattern in OO (recitation)
- Implementation of grammars via OOP 


Some logistics:
- PS1 is due tonight at midnight
- PS2 is out tomorrow evening; due on Monday, June 8th at midnight
- Project 1 is coming out on Friday and will be due in two weeks time (from that Friday)
- No lecture on Thursday, as I will be attending a conference.  A recording will be available on Canvas

Random notes:
- If this is something that interests you, CSCI 4448/5448: Object Oriented Analysis and Design (OOAD) would be up your alley